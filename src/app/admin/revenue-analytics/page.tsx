...
export default function RevenueAnalytics() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Map total_price to totalPrice for compatibility
  ...
  // Calculate revenue for filtered bookings
  let chartData: { label: string; revenue: number }[] = [];
  if (filter === 'year') {
    // Group by month
    const monthly: Record<string, number> = {};
    filteredBookings.forEach(b => {
      const month = dayjs((b as any).start_date).format('YYYY-MM');
      monthly[month] = (monthly[month] || 0) + (b as any).totalPrice;
    });
    chartData = Object.entries(monthly).map(([month, revenue]) => ({ label: month, revenue }));
  } else if (filter === 'month') {
    // Group by day
    const daily: Record<string, number> = {};
    filteredBookings.forEach(b => {
      const day = dayjs((b as any).start_date).format('YYYY-MM-DD');
      daily[day] = (daily[day] || 0) + (b as any).totalPrice;
    });
    chartData = Object.entries(daily).map(([day, revenue]) => ({ label: day, revenue }));
  } else if (filter === 'day') {
    // Group by booking (single day)
    chartData = filteredBookings.map(b => ({ label: dayjs((b as any).start_date).format('YYYY-MM-DD'), revenue: (b as any).totalPrice }));
  }
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-gray-600">Monthly revenue overview</p>
              </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-2">Monthly Revenue</h2>
              <p className="text-gray-500 mb-4 text-sm">
                This chart shows total revenue from approved bookings, grouped by month.
              </p>
              {chartData.length === 0 ? (
                <div className="text-center text-gray-400 py-12">No revenue data available.</div>
              ) : (
